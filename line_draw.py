...
def load_poly_area_data(img_name):
    """加载对用图片多边形点数据 :param img_name: 图片名称 :return: 多边形的坐标 [[x1,y1],[x2,y2],……,[xn,yn],[x1,y1]] 二维数组."""
...
def draw_poly_area_dangerous(img, img_name, throughJSON=True):
    """画多边形危险区域的框 :param img: 图像本身 :param img_name:用于加载绘制区域的json路径 / 一对点的x和y list组成的list :param done:是否需要执行以下代码
    :return:
    """
    if throughJSON:
...
def is_poi_in_poly(pt, poly):
    """判断点是否在多边形内部的 pnpoly 算法 :param pt: 点坐标 [x,y] :param poly: 点多边形坐标 [[x1,y1],[x2,y2],...] :return: 点是否在多边形之内."""
    nvert = len(poly)
...
def person_in_poly_area_dangerous(xyxy, area_poly):
    """检测人体是否在多边形危险区域内 :param xyxy: 人体框的坐标 :param img_name: 检测的图片标号，用这个来对应图片的危险区域信息 :return: True -> 在危险区域内，False ->
    不在危险区域内.
    """
    # area_poly = load_poly_area_data_simple(img_name)
    # print(area_poly)
    if area_poly is None:  # 为空
        return False
    # 求物体框的中点
    object_x1 = int(xyxy[0])
    object_y1 = int(xyxy[1])
